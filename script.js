...
const lightStory={
    ...
    t:'按高端4A TVC标准，在30秒/16镜头内建立可识别的品牌世界，并让观众从“看见光”自然过渡到“感受肌肤光泽”。',
    ...
    r:'形成一套可直接进入提案/生成流程的30秒TVC方案：产品机制、使用动作、肌肤结果和品牌落款被压缩为一条清晰的感官叙事；文件夹内同时保留成片、产品基准图和提案视觉板，便于展示从品牌策略到画面执行的闭环。'
};
const velocityStory={
...
function getProjectView(key,p){
  return {
    title:key==='velocity'?'01｜联想小新 Pro14GT 产品广告':key==='light'?'03｜娇韵诗 Double Serum 30秒 TVC':key==='boiling'?'04｜浓香麻辣火锅底料广告':key==='sixlooks'?'05｜多风格女装一秒变装短视频':key==='spring'?'06｜春日女装分屏对比展示片':key==='lucky'?'07｜3D短剧《五岁小福星，顾家宠上天》':p.title,
